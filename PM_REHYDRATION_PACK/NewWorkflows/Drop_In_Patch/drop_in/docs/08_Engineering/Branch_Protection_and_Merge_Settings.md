...
Last verified: 2026-01-15 — Updated for risk-based gated checks (Bugbot + Codecov + Claude).
Status: Canonical
...
- bypassing CI
- merging without risk-appropriate semantic/coverage gates
...
- ✅ Required checks must include both:
  - Baseline CI (fast, every commit)
  - Policy Gate (enforces risk-based gated checks)

This repo’s baseline workflow + job:
- `.github/workflows/ci.yml` → workflow name: `CI`, job: **`validate`** (strict/up-to-date = **true**)

Policy gate workflow + job:
- `.github/workflows/policy-gate.yml` → workflow name: `Policy Gate`, job: **`policy-gate`**

Important:
- GitHub only lets you select required checks after the workflow has run at least once.
- Create a small PR to trigger workflows, then return here and select the check(s) from the list.
...
1) Create a PR from a run branch (example): `run/RUN_20260115_2315Z` → `main`

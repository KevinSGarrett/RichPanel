...
- `risk:R0` — docs/comments only, no runtime impact
- `risk:R1` — low risk (minor behavior changes, non-critical paths)
- `risk:R2` — medium risk (core logic / integrations)
- `risk:R3` — high risk (security, PII, production safety, infra)
- `risk:R4` — critical (auth, secrets, compliance, payment-like flows)
...
If new commits are pushed after gates, the repo will label the PR `stale:gates`.

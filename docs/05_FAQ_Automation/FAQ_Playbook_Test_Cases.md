...
- Auto-close only for whitelisted, deflection-safe templates (CR-001 adds order-status ETA exception)
...
### TC-OS-03: Order linked, no tracking yet (within SLA window)
**Input:** “What’s the status of my order?”  
**Precondition:** deterministic match true; tracking_url missing; shipping bucket recognized; order is within SLA window  
**Expected:** Tier 2 auto-send + auto-close eligible  
- template: `t_order_eta_no_tracking_verified`
- vars: `shipping_bucket`, `eta_remaining_human` computed from order date + SLA
- tags: `intent:order_status`, `mw:auto_sent`, `mw:auto_closed`
- route: none (resolve conversation)

**Notes:** must include reply-to-reopen language; no invented tracking.

### TC-OS-04: Order linked, no tracking, outside SLA window (late)
**Input:** “My order still hasn’t arrived.”  
**Precondition:** deterministic match true; tracking_url missing; elapsed business days >= SLA max  
**Expected:** Tier 2 escalation-safe response + route to human (no auto-close)  
- template: `t_shipping_delay_verified`
- tags: `intent:shipping_delay`, `mw:auto_sent`

...
|---|---|---|
| `tracking_url` exists OR fulfillment has tracking | `t_order_status_verified` | Customer likely wants tracking; we can provide it when linked. |
| No tracking + fulfillment not shipped | `t_shipping_delay_verified` | We can safely state “not shipped yet” and route. |
...
### Preorder / backorder
- If order status indicates “preorder/backorder” in status text:
  - Treat as shipping delay: `t_shipping_delay_verified`
  - Route to Email Support Team for a human follow-up (preorders often need context).

...
Last verified: 2025-12-22 — Wave 11 complete (Update 2).
...
15) ✅ Auto-close policy: middleware must **auto-close only for whitelisted, deflection-safe templates (CR-001 adds order-status ETA exception)**.
...
32) ✅ Analytics export posture decided (v1):
   - CloudWatch-first is **required** for operations
   - sanitized S3 export (CloudWatch Logs → Firehose → S3) is **optional**
   - Best-suggested rollout: enable in **staging** early; enable in **prod** after first rollout window if comfortable
33) 🟡 Will Support Ops adopt the **MW feedback macros/tags** (misroute / auto-reply wrong/helpful)?
   - Recommended: yes for early rollout; it’s the highest-signal quality loop.
...
## Wave 09 — Testing/Release readiness questions

1) 🟡 Do we have (or want) a **separate Richpanel staging workspace**?
   - Best: separate workspace + separate API key.
   - Fallback: same workspace with `mw-test` tag allowlist, or full stubs in staging.
...
## F) Governance ownership and cadence
1) 🟡 Who is the **Support Ops Owner** responsible for approving customer-facing template copy and macro alignment?
   - Default assumption: a designated Support lead (Email/LiveChat ops), with Leadership informed.

2) 🟡 Who is the **Quality/Eval Owner** responsible for weekly triage and monthly calibration runs?
   - Default assumption: Engineering initially, then transition to an analyst/ops role as volume grows.

3) 🟡 Who receives the **monthly calibration report** and has authority to approve taxonomy changes?
   - Default assumption: Engineering + Support Ops, Leadership informed.

4) 🟡 Do you want to enable **sanitized S3 analytics export** in production v1 or only in staging?
   - Default recommendation: staging first; production after initial rollout stability.

...
| Richpanel bot agent id | Required for outbound identity in email-channel replies (see reply-path docs). | Unique per env; prod required for outbound identity. | Secrets Manager `rp-mw/<env>/richpanel/bot_agent_id` (or env var override when explicitly configured). | Owner: CX Ops / Workspace admin. **Pending:** create prod secret if missing and document source of truth. |
...
- **Preflight enforcement (PII-safe):**
  - `scripts/aws_account_preflight.py` validates account + region before any run.
  - `scripts/secrets_preflight.py` validates Secrets Manager + SSM kill-switch params.

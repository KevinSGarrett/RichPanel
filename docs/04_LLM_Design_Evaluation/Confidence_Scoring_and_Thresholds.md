...
### D) Auto-close thresholds (deflection-safe)
Auto-close is not a general feature in v1. It is allowed only for a small whitelist of deflection-safe templates.

Default requirements (in addition to Tier 2 eligibility):
- `template_id` is in the auto-close allowlist (initial: `t_order_eta_no_tracking_verified`)
- order is within its SLA window (computed `remaining_max > 0`)
- reply includes "reply to reopen" language

If any condition fails:
- send Tier 2 without auto-close (optional), or
- route to a human (preferred if late/out-of-window)

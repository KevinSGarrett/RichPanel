...
Last verified: 2025-12-29 — Updated to v1 `route-*` tag strategy and Chargebacks/Disputes queue naming.
...
4) Disable/modify conflicting existing automations:
   ...
   - This conflicts with our hard constraint: **middleware does not auto-close by default** (auto-close is whitelisted only for deflection-safe order status ETA; CR-001). Chargebacks/disputes must never be auto-closed.

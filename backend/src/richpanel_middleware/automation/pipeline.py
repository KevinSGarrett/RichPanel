...
def _ensure_order_status_greeting(body: str, first_name: Optional[str]) -> str:
    ...
    if greeting_match:
        ...
        if remainder:
            ...
            if not lines[1:] or lines[1].strip():
                updated.append("")
        updated.extend(lines[1:])
        lines = updated
    else:
...
def _build_order_status_reply_context(
    *,
    ...
    if delivery_estimate_payload is not None:
        ...
        late_flag = delivery_estimate_payload.get("is_late")
        if late_flag is not None:
            is_late = bool(late_flag)
...
_REWRITE_REASON_ERROR_CLASS = {
    ...
    "missing_required_dates": "OpenAIInvariantViolation",
    ...
    "unexpected_dates": "OpenAIInvariantViolation",

...
from richpanel_middleware.automation.llm_routing import (
    DEFAULT_CONFIDENCE_THRESHOLD, LLMRoutingSuggestion, OPENAI_ROUTING_PRIMARY_DEFAULT,
    compute_dual_routing, get_confidence_threshold, get_openai_routing_primary, suggest_llm_routing,
)
from integrations.openai.client import ChatCompletionRequest, ChatCompletionResponse
...
class MockOpenAIClient:
    ...
    def chat_completion(self, request, *, safe_mode, automation_enabled):
        ...
        if self.dry_run or safe_mode or not automation_enabled:
            return ChatCompletionResponse(model=request.model, message=None, status_code=0,
                url='test', raw={'reason': 'dry_run'}, dry_run=True, reason='dry_run')
        return ChatCompletionResponse(model=request.model, message=json.dumps(self.response_json),
            status_code=200, url='test', raw=self.response_json, dry_run=False)


class GatingTests(unittest.TestCase):
    def test_gating_blocks_safe_mode(self):
        client = MockOpenAIClient()
        suggestion = suggest_llm_routing(customer_message='test', conversation_id='c', event_id='e',
            safe_mode=True, automation_enabled=True, allow_network=True, outbound_enabled=True, client=client)
    ...
    def test_gating_blocks_automation_disabled(self):
        client = MockOpenAIClient()
        suggestion = suggest_llm_routing(customer_message='test', conversation_id='c', event_id='e',
            safe_mode=False, automation_enabled=False, allow_network=True, outbound_enabled=True, client=client)
        self.assertEqual(client.call_count, 0)
    ...
    def test_gating_blocks_network_disabled(self):
        client = MockOpenAIClient()
        suggestion = suggest_llm_routing(customer_message='test', conversation_id='c', event_id='e',
            safe_mode=False, automation_enabled=True, allow_network=False, outbound_enabled=True, client=client)
        self.assertEqual(client.call_count, 0)
        self.assertEqual(suggestion.gated_reason, 'network_disabled')

    def test_gating_allows_all_enabled(self):
        client = MockOpenAIClient(response_json={'intent': 'order_status_tracking', 'department': 'Email Support Team', 'confidence': 0.95})
        suggestion = suggest_llm_routing(customer_message='test', conversation_id='c', event_id='e',
            safe_mode=False, automation_enabled=True, allow_network=True, outbound_enabled=True, client=client)
...
class ArtifactTests(unittest.TestCase):
    def test_artifact_contains_deterministic(self):
        ...
        routing, artifact = compute_dual_routing(payload={'customer_message': 'Where is my order?'},
            conversation_id='c', event_id='e', safe_mode=True, automation_enabled=True,
            allow_network=False, outbound_enabled=False, client=client)
        self.assertIn('intent', artifact.deterministic)

    def test_artifact_serializable(self):
        client = MockOpenAIClient(dry_run=True)
        routing, artifact = compute_dual_routing(payload={'customer_message': 'test'},
            conversation_id='c', event_id='e', safe_mode=False, automation_enabled=True,
            allow_network=False, outbound_enabled=False, client=client)
...
class PrimaryFlagTests(unittest.TestCase):
    ...
    def test_default_uses_deterministic(self):
        client = MockOpenAIClient(response_json={'intent': 'refund_request', 'department': 'Returns Admin', 'confidence': 0.99})
        routing, artifact = compute_dual_routing(payload={'customer_message': 'Where is my order?'},
            conversation_id='c', event_id='e', safe_mode=False, automation_enabled=True,
            allow_network=True, outbound_enabled=True, client=client)
...
class PrimaryFlagOnTests(unittest.TestCase):
    ...
    def tearDown(self):
        if self._orig_p: os.environ['OPENAI_ROUTING_PRIMARY'] = self._orig_p
        else: os.environ.pop('OPENAI_ROUTING_PRIMARY', None)
        if self._orig_t: os.environ['OPENAI_ROUTING_CONFIDENCE_THRESHOLD'] = self._orig_t
        else: os.environ.pop('OPENAI_ROUTING_CONFIDENCE_THRESHOLD', None)

    def test_high_confidence_uses_llm(self):
        client = MockOpenAIClient(response_json={'intent': 'refund_request', 'department': 'Returns Admin', 'confidence': 0.95})
        routing, artifact = compute_dual_routing(payload={'customer_message': 'refund please'},
            conversation_id='c', event_id='e', safe_mode=False, automation_enabled=True,
            allow_network=True, outbound_enabled=True, client=client)
    ...
    def test_low_confidence_uses_deterministic(self):
        client = MockOpenAIClient(response_json={'intent': 'refund_request', 'department': 'Returns Admin', 'confidence': 0.5})
        routing, artifact = compute_dual_routing(payload={'customer_message': 'Where is my order?'},
            conversation_id='c', event_id='e', safe_mode=False, automation_enabled=True,
            allow_network=True, outbound_enabled=True, client=client)
        self.assertEqual(artifact.primary_source, 'deterministic')
...
class SuggestionTests(unittest.TestCase):
    def test_is_valid_with_valid_data(self):
        s = LLMRoutingSuggestion(intent='order_status_tracking', department='Email Support Team', confidence=0.9)
        self.assertTrue(s.is_valid())

    def test_is_valid_with_invalid_intent(self):
        s = LLMRoutingSuggestion(intent='not_real', department='Email Support Team', confidence=0.9)
...
class ThresholdTests(unittest.TestCase):
    ...
    def tearDown(self):
        if self._orig: os.environ['OPENAI_ROUTING_CONFIDENCE_THRESHOLD'] = self._orig
        else: os.environ.pop('OPENAI_ROUTING_CONFIDENCE_THRESHOLD', None)

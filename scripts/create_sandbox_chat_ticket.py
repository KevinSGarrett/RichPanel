...
ENV_CHANNEL = "MW_SMOKE_TICKET_CHANNEL"
...
def _build_ticket_payload(
    *,
    ...
    via_payload: Dict[str, Any] = {"channel": channel}
    if channel == "email":
        via_payload["source"] = {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create a fresh sandbox non-email ticket (PII-safe output)."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--channel",
        help=f"Ticket channel to use (default: {DEFAULT_CHANNEL}).",
    )
    parser.add_argument(
        "--from-email",
        dest="from_email",
...
def main() -> int:
    ...
    channel = _resolve_text(args.channel, ENV_CHANNEL, DEFAULT_CHANNEL).lower()
    ...
    if response.status_code < 200 or response.status_code >= 300:
        ...
        if error_body is None:
            ...
            if message:
                snippet = _redact_emails(str(message))
            elif errors:
                snippet = _redact_emails(str(errors))
            else:
                snippet = _redact_emails(str(list(error_body.keys())))
        else:
            snippet = _redact_emails(str(error_body))
        print(
            f"[FAIL] Ticket creation failed with status {response.status_code}: {snippet}",
            file=sys.stderr,
        )
        return 1
    ...
    print(
        "[OK] Created sandbox non-email ticket "

...
class ReadOnlyShadowModeTests(unittest.TestCase):
    ...
    def test_plan_actions_propagates_allow_network_for_shadow_reads(self) -> None:
        envelope = EventEnvelope(
            ...
            group_id="grp-shadow-1",
            dedupe_id="dedupe-shadow-1",
        ...
        self.assertTrue(
            any(
                action.get("type") == "order_status_draft_reply"
    ...
    def test_outbound_disabled_skips_writes_while_allowing_shadow_reads(self) -> None:
        ...
        envelope = EventEnvelope(
            event_id="evt-shadow-2",
            received_at="2024-01-01T00:00:00Z",
            group_id="grp-shadow-2",
            dedupe_id="dedupe-shadow-2",
            payload={},
            source="test",
            conversation_id="c-shadow-2",
        )
        ...
        with mock.patch.object(
            pipeline, "_resolve_target_ticket_id", return_value="c-shadow-2"
        ), mock.patch.object(
            pipeline,
            "_safe_ticket_metadata_fetch",
            return_value=SimpleNamespace(status=None, tags=[]),
        ), mock.patch.object(

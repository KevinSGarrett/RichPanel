...
- Added AWS SDK HTTP tracing + operation allowlist to provide full network proof in `live_readonly_shadow_eval.py`.
- Added `SHOPIFY_ACCESS_TOKEN_PROFILE` support to load Shopify secrets via a secondary AWS profile in `live_readonly_shadow_eval.py`.
...
- Updated PR gate evidence links + Claude gate response id in run docs.

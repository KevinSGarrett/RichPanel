...
- Retried shadow eval via workflow and per-ticket local runs; workflow blocked by disabled OpenAI routing, local runs used /v1/tickets only (skip conversations) with Shopify ok; order_status candidate + OpenAI calls observed (rewrite inference noted).

# Git Run Plan

Use this file to coordinate Git/GitHub execution for the run.

**RUN_ID:** RUN_20260112_2030Z  
**Mode:** sequential  
**Integrator:** C  
**Target branch:** `run/RUN_20260112_2030Z_payload_first_order_lookup`  
**Merge strategy:** merge commit (locked)  
**Branch cleanup:** yes (required)  

---

## Main branch rule
- `main` is protected: changes land via PR (required status checks; merge commit).

## Branch plan
### Sequential (default)
- All agents use: `run/<RUN_ID>`

### Parallel (only when scopes are disjoint)
- Agent A: `run/<RUN_ID>-A`
- Agent B: `run/<RUN_ID>-B`
- Agent C: `run/<RUN_ID>-C`
- Integrator merges into `run/<RUN_ID>`

---

## Agent scopes and locks (required)

### Agent A
- Allowed paths:
  - none (not staffed this run)
- Locked paths (do not edit):
  - all

### Agent B
- Allowed paths:
  - none (not staffed this run)
- Locked paths:
  - all

### Agent C
- Allowed paths:
  - `backend/src/richpanel_middleware/commerce/order_lookup.py`
  - `scripts/test_order_lookup.py`
  - `scripts/fixtures/order_lookup/*`
  - `REHYDRATION_PACK/RUNS/RUN_20260112_2030Z/C/*`
- Locked paths:
  - everything else

---

## Integration checklist (Integrator)
- [x] Pull latest `main`
- [x] Merge agent branches (if parallel) into `run/<RUN_ID>` (not needed; single branch)
- [x] Run: `python scripts/run_ci_checks.py --ci`
- [x] Merge `run/<RUN_ID>` → `main` (PR opened with auto-merge)
- [x] Confirm Actions are green (or document failure + fix)
- [ ] Delete run branches + agent branches (will complete after merge)
- [ ] Update: `REHYDRATION_PACK/GITHUB_STATE.md`

...
- `python -c "from backend.src.richpanel_middleware.automation import delivery_estimate as d; import inspect; print('has_fallback', '_preorder_delivery_fallback_window' in inspect.getsource(d))"` - initial B83 probe.
- `rg "compute_preorder_delivery_estimate" backend\src\richpanel_middleware\automation` - confirm preorder fallback logic present.
- `git rev-parse HEAD` - record main SHA.
...
- `gh run watch 22048267858 --exit-status` - wait for deploy.
- `gh run view 22048267858 --json url --jq ".url" > REHYDRATION_PACK/RUNS/RUN_20260216_0238Z/B/deploy_prod_run_url.txt` - capture run URL.
...
- B83 presence proof: `rg "compute_preorder_delivery_estimate" backend\src\richpanel_middleware\automation`
- HEAD SHA: `278f820fe91e15bedc04c0c1284048ebd607c7e3`
...
- Deploy run URL: https://github.com/KevinSGarrett/RichPanel/actions/runs/22048267858
